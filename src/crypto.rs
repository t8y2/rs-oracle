...
pub fn encrypt_cbc_192(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>> {
    ...
    // Pad to block size (16 bytes) — skip padding when already aligned
    let block_size = 16;
    let remainder = plaintext.len() % block_size;
...
/// Encrypt data using AES-128-CBC with zero IV (for 10g verifier 2361)
pub fn encrypt_cbc_128(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>> {
    if key.len() != 16 {
        return Err(Error::Protocol(format!(
    ...
    let cipher = Aes128CbcEnc::new(key.into(), &ZERO_IV.into());
    cipher
        .encrypt_padded_mut::<NoPadding>(&mut buffer, msg_len + padding_len)
        .map_err(|e| Error::Protocol(format!("AES encryption failed: {}", e)))?;
...
/// Decrypt data using AES-128-CBC with zero IV (for 10g verifier 2361)
pub fn decrypt_cbc_128(key: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>> {
    if key.len() != 16 {
        return Err(Error::Protocol(format!(
            "AES-128 key must be 16 bytes, got {}",
            key.len()
    ...
    let cipher = Aes128CbcDec::new(key.into(), &ZERO_IV.into());
    let mut buffer = ciphertext.to_vec();
    cipher
        .decrypt_padded_mut::<NoPadding>(&mut buffer)
        .map_err(|e| Error::Protocol(format!("AES decryption failed: {}", e)))?;
...
/// Encrypt data using AES-128-CBC with zero IV, no padding (for session keys).
///
/// Plaintext must already be block-aligned (multiple of 16 bytes).
pub fn encrypt_cbc_128_raw(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>> {
    if key.len() != 16 {
        return Err(Error::Protocol(format!(
            "AES-128 key must be 16 bytes, got {}",
            key.len()
    ...
    if plaintext.len() % 16 != 0 {
        return Err(Error::Protocol(format!(
            "Plaintext must be block-aligned for raw encrypt, got {} bytes",
            plaintext.len()
        )));
    }
...
/// Generate 10g-style password key from username and password using DES.
///
/// This is the key derivation used by Oracle 10g (verifier type 2361).
/// It uppercases and extends both username and password to UTF-16 style
/// (zero + char byte), concatenates them, then runs DES encryption twice
/// with a fixed initial key.
pub fn generate_10g_password_key(username: &str, password: &[u8]) -> Vec<u8> {
    let upper_user = username.to_uppercase();
    let upper_pass = String::from_utf8_lossy(password).to_uppercase();

    // Extend to 2-byte-per-char (UTF-16 style: 0x00 + char byte)
    let extend = |s: &str| -> Vec<u8> {
    ...
    let mut buffer = extend(&upper_user);
    buffer.extend_from_slice(&extend(&upper_pass));

    // Pad to 8-byte boundary
    let rem = buffer.len() % 8;
    if rem != 0 {
        buffer.extend(std::iter::repeat(0u8).take(8 - rem));
    }

    // Fixed initial key
    let key = [1u8, 35, 69, 103, 137, 171, 205, 239];

    let des_xor_encrypt = |input: &[u8], des_key: &[u8]| -> Vec<u8> {
        use des::cipher::{BlockEncrypt, KeyInit};
        let cipher = des::Des::new_from_slice(des_key).expect("DES key must be 8 bytes");
        let mut accumulator = [0u8; 8];
        for chunk in input.chunks(8) {
            for i in 0..8 {
                accumulator[i] ^= chunk[i];
            }
            cipher.encrypt_block((&mut accumulator).into());
        }
        accumulator.to_vec()
    };

    let key1 = des_xor_encrypt(&buffer, &key);
    let key2 = des_xor_encrypt(&buffer, &key1);

    // Return 16 bytes: key2 + 8 zero bytes
    let mut result = key2;
