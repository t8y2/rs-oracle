...
impl Error {
    ...
    /// Check if this error is recoverable (can retry the connection/operation)
    pub fn is_recoverable(&self) -> bool {
        matches!(
            ...
            Error::ConnectionTimeout(_)
                | Error::ConnectionClosed
                | Error::ConnectionClosedByServer(_)
        ) || matches!(self, Error::OracleError { code, .. } if matches!(
            *code,
            crate::constants::error_code::END_OF_FILE_ON_COMM
                | crate::constants::error_code::CONNECTION_LOST_CONTACT
                | crate::constants::error_code::SESSION_KILLED
                | crate::constants::error_code::EXCEEDED_IDLE_TIME
        ))
    }

    /// Check if this is a unique constraint violation (ORA-00001)
    pub fn is_unique_violation(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if *code == crate::constants::error_code::UNIQUE_VIOLATION)
    }

    /// Check if this is a lock timeout (ORA-04021 or ORA-00060)
    pub fn is_lock_timeout(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if matches!(
            *code,
            crate::constants::error_code::LOCK_TIMEOUT | crate::constants::error_code::DEADLOCK
        ))
    }

    /// Check if this is a deadlock detected by Oracle (ORA-00060)
    pub fn is_deadlock(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if *code == crate::constants::error_code::DEADLOCK)
    }

    /// Check if this is a SQL syntax error (ORA-00900~00999, ORA-01756, etc.)
    pub fn is_syntax_error(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if
            *code == crate::constants::error_code::INVALID_SQL_STATEMENT
            || *code == crate::constants::error_code::QUOTED_STRING_NOT_TERMINATED
            || (900..=999).contains(code)
        )
    }

    /// Check if this is an insufficient privileges error (ORA-01031)
    pub fn is_insufficient_privileges(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if *code == crate::constants::error_code::INSUFFICIENT_PRIVILEGES)
    }

    /// Check if this is a table or view not found error (ORA-00942)
    pub fn is_table_not_found(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if *code == crate::constants::error_code::TABLE_OR_VIEW_NOT_FOUND)
    }

    /// Check if this is a sequence not found error (ORA-02289)
    pub fn is_sequence_not_found(&self) -> bool {
        matches!(self, Error::OracleError { code, .. } if *code == crate::constants::error_code::SEQUENCE_NOT_FOUND)
    }
}
