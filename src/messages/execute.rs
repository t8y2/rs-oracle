...
impl ExecuteOptions {
    ...
    pub fn for_query(prefetch_rows: u32) -> Self {
        Self {
            ...
            prefetch_rows,
            num_execs: 1,
            scrollable: prefetch_rows > 0, // keep cursor open for re-fetch
            ..Default::default()
        }
    }
...
impl<'a> ExecuteMessage<'a> {
    ...
    pub fn build_request_with_sdu(&self, caps: &Capabilities, large_sdu: bool) -> Result<Bytes> {
        self.build_request_inner(caps, large_sdu, None)
    }

    /// Build the execute request packet with borrowed bind params (zero-clone hot path).
    /// Takes `&[Value]` directly, avoiding `params.to_vec()` allocation.
    pub fn build_request_with_params(
        ...
        self.build_request_inner(caps, large_sdu, Some(params))
    }

    /// Internal: build request, optionally using borrowed params instead of stored batch_bind_values.
    fn build_request_inner(
        ...
        match self.function_code {
            FunctionCode::Execute => self.write_execute_message_inner(&mut buf, caps, params)?,
            FunctionCode::Reexecute | FunctionCode::ReexecuteAndFetch => {
                self.write_reexecute_message_inner(&mut buf, caps, params)?
            }
            _ => unreachable!(),
        }
        ...
        // Data flags at offset 8 (go-ora DataPacket format)
        packet.put_u16(0);
    ...
    /// Write a 10g-compatible execute message body matching Java JDBC thin driver format.
    ///
    /// This function reproduces the Java thin driver's execute message byte-for-byte for
    /// Oracle 10g (ttc_fv <= 5). Key differences from 11g+:
    /// - SQL text written as raw bytes (no PutClr/CLR length prefix)
    /// - Prefetch uses PutUint(0xFFFFFFFF, 4) + PutUint(rows, 4) format
    /// - max_long = 0x7FFFFFFF (non-INLINE mode)
    /// - al8i4[7] = 0 for SELECT (not 1)
    /// - al8i4[10] = 0x8000 (not 640)
    fn write_execute_message_10g(
        &self,
        buf: &mut WriteBuffer,
        _caps: &Capabilities,
        exec_opts: u32,
        ...
        let is_parse = stmt.cursor_id() == 0 || stmt.is_ddl();

        // TTC header: PutBytes(3, 0x5E, seq)
        buf.write_u8(MessageType::Function as u8)?;
        buf.write_u8(self.function_code as u8)?;
        buf.write_u8(self.sequence_number)?;

        // exec_opts: PutUint(exeOp, 4, true, true)
        buf.write_ub4(exec_opts)?;

        // cursor_id: PutUint(cursorID, 2, true, true) — UB2
        buf.write_ub2(stmt.cursor_id() as u16)?;

        // parse_ptr: PutBytes(1) if cursorID==0, else PutBytes(0)
        if stmt.cursor_id() == 0 {
            buf.write_u8(1)?;
        } else {
            buf.write_u8(0)?;
        ...
        // SQL length + parse flag
        if is_parse {
            buf.write_ub4(stmt.sql_bytes().len() as u32)?;
            buf.write_u8(1)?; // PutBytes(1)
        } else {
            buf.write_u8(0)?;
            buf.write_u8(1)?; // PutBytes(0, 1)
        }

        // al8i4 vector length: PutUint(13, 2, true, true) — UB2
        buf.write_ub2(13u16)?;

        // al8o4 + al8o4l: PutBytes(0, 0)
        buf.write_u8(0)?;
        buf.write_u8(0)?;

        // Prefetch section: matches go-ora basicWrite format.
        // When PARSE+EXECUTE+SELECT (no FETCH): PutBytes(0) + PutUint(rows, 4)
        // Otherwise: PutUint(0, 4) + PutUint(0, 4)
        // Condition: FETCH not set && EXECUTE set && PARSE set && SELECT
        if exec_opts & exec_option::FETCH == 0
            && exec_opts & exec_option::EXECUTE != 0
            && is_parse
            && stmt.is_query()
        {
            buf.write_u8(0)?; // PutBytes(0) — go-ora compat (single zero byte)
            buf.write_ub4(num_iters)?; // PutUint(rows, 4)
        } else {
            buf.write_ub4(0u32)?; // PutUint(0, 4)
            buf.write_ub4(0u32)?; // PutUint(0, 4)
        }

        // max_long: 0x7FFFFFFF (Java non-INLINE mode)
        buf.write_ub4(0x7FFF_FFFFu32)?;

        // Bind params count
        if num_params > 0 {
            buf.write_u8(1)?;
            buf.write_ub2(num_params as u16)?; // PutUint(count, 2, true, true)
        } else {
            buf.write_u8(0)?;
            buf.write_u8(0)?; // PutBytes(0, 0)
        }

        // 5 zeros: PutBytes(0, 0, 0, 0, 0)
        buf.write_u8(0)?;
        buf.write_u8(0)?;
        buf.write_u8(0)?;
        buf.write_u8(0)?;
        ...
        // SQL text: Java writes raw bytes (no PutClr length prefix)
        // go-ora uses PutClr, but Java thin driver for 10g uses raw encoding.
        // We match Java since it's proven to work with the target server.
        if is_parse {
            let sql_bytes = stmt.sql_bytes();
            buf.write_bytes(sql_bytes)?;
        }

        // --- al8i4 array (13 entries, each PutUint(_, 4, true, true)) ---
        // Values verified against Java JDBC thin driver trace for Oracle 10g:
        //   [0]=1(parse) [1]=0(exec_cnt for SELECT+parse) [2-6]=0(SCN)
        //   [7]=1(is_query) [8]=0 [9]=0x8000(exec_flgs) [10-12]=0
        //
        // al8i4[0]: parse flag
        buf.write_ub4(if is_parse { 1 } else { 0 })?;

        // al8i4[1]: execution count (Java: 0 for SELECT+parse)
        if stmt.is_query() {
            buf.write_ub4(0)?;
        } else if stmt.is_plsql() {
            buf.write_ub4(opts.num_execs.max(1))?;
        } else {
            buf.write_ub4(1)?;
        }
        ...
        // al8i4[7]: 1 for SELECT queries (matches Java & go-ora)
        buf.write_ub4(if stmt.is_query() { 1 } else { 0 })?;

        // al8i4[8]: 0 (Java compat)
        buf.write_ub4(0)?;

        // al8i4[9]: execute flags (Java = 0x8000 when EXECUTE set)
        buf.write_ub4(exec_flgs)?;

        // al8i4[10]: 0 (Java compat)
        buf.write_ub4(0)?;
    ...
    /// Write a full execute message, optionally using borrowed params.
    fn write_execute_message_inner(
        &self,
        buf: &mut WriteBuffer,
        caps: &Capabilities,
        params: Option<&[Value]>,
        ...
        if stmt.requires_define() {
            ...
            // Only set EXECUTE when we have SQL.
            // REF CURSORs have no SQL and should not set this flag.
            // al8i4[9] |= 0x8000 when EXECUTE is set (go-ora / Java compat, all versions).
        ...
        // SCROLLABLE and NO_CANCEL_ON_EOF are 12c+ features, skip for 10g/11g
        if opts.scrollable && caps.ttc_field_version >= ccap_value::FIELD_VERSION_12_1 {
        ...
        // Oracle 10g (ttc_fv <= 5): FETCH is mutually exclusive with PARSE+EXECUTE.
        // When both PARSE and EXECUTE are set, fetch happens implicitly via prefetch_rows.
        if opts.describe_only {
            ...
            if caps.ttc_field_version <= ccap_value::FIELD_VERSION_10_2 {
                // 10g: only set FETCH when NOT also parsing (go-ora compatible)
                if stmt.cursor_id() != 0 && !stmt.is_ddl() {
        ...
        let mut num_iters = if stmt.is_query() {
            opts.prefetch_rows
        } else {
            opts.num_execs
        };
        // 10g: cap prefetch at 25 (go-ora default) to fit in 1-byte UB4 value
        if caps.ttc_field_version <= ccap_value::FIELD_VERSION_10_2 {
            num_iters = num_iters.min(25);
        }

        // Dispatch to 10g-specific writer (go-ora basicWrite compat)
        // Bind params are written after the 10g header, same format as non-10g
        if caps.ttc_field_version <= ccap_value::FIELD_VERSION_10_2 {
            self.write_execute_message_10g(
                buf, caps, exec_opts, exec_flgs, num_params, num_iters,
            )?;
            // Write bind parameter data
            if let Some(params) = params {
                if !params.is_empty() {
                    self.write_single_row_bind_params(buf, params, caps)?;
                }
            } else if self.has_bind_values() {
        ...
        // Write execute body (11g+ path; 10g is handled by write_execute_message_10g)
        ...
        // Registration and extended fields
        ...
        if stmt.cursor_id() == 0 || stmt.is_ddl() {
            let sql_bytes = stmt.sql_bytes();
            buf.write_bytes_with_length_ext(Some(sql_bytes), caps.use_big_clr_chunks)?;
            ...
            for (col_idx, value) in row.iter().enumerate() {
                let size = match value {
                    ...
                    Value::Json(json) => {
                        ...
                        serde_json::to_writer(&mut cw, json)
                            .map(|_| cw.0 as u32)
                            .unwrap_or(100)
                    }
    ...
    /// Write single-row bind parameters directly from a borrowed slice,
    /// avoiding the params.to_vec() clone and intermediate Vec<Vec<Value>> storage.
    /// This is the hot path for non-batch query/DML execution.
    pub fn write_single_row_bind_params(
        &self,
        buf: &mut WriteBuffer,
        params: &[Value],
        caps: &Capabilities,
        ...
        // Calculate max sizes from the single row
        let mut max_sizes: Vec<u32> = Vec::with_capacity(params.len());
        for value in params {
            let size = match value {
                Value::String(s) => s.len() as u32,
                Value::Bytes(b) => b.len() as u32,
        ...
        // Write metadata for each bind parameter
        for (col_idx, value) in params.iter().enumerate() {
            let (oracle_type, buffer_size, csfrm, cont_flag) =
                self.infer_bind_metadata(value, max_sizes[col_idx]);

            buf.write_u8(oracle_type as u8)?;
    ...
    fn write_bind_value(&self, buf: &mut WriteBuffer, value: &Value, big_clr: bool) -> Result<()> {
        ...
        match value {
            ...
            Value::IntervalYm(interval) => {
                use crate::types::encode_interval_ym;
                let bytes = encode_interval_ym(interval);
                buf.write_u8(bytes.len() as u8)?;
                buf.write_bytes(&bytes)?;
            }
            Value::IntervalDs(interval) => {
                use crate::types::encode_interval_ds;
                let bytes = encode_interval_ds(interval);
                buf.write_u8(bytes.len() as u8)?;
                buf.write_bytes(&bytes)?;
            }
        }
    ...
    fn write_reexecute_message_inner(
        &self,
        buf: &mut WriteBuffer,
        caps: &Capabilities,
        params: Option<&[Value]>,
        ...
        let num_iters = if self.function_code == FunctionCode::ReexecuteAndFetch {
            options_1 |= exec_option::EXECUTE | exec_option::FETCH;
            opts.prefetch_rows
        } else {
            if opts.commit {
