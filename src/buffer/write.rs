...
impl WriteBuffer {
    ...
    /// Write bytes with a length prefix and optional extended (big CLR) encoding.
    pub fn write_bytes_with_length_ext(
        &mut self,
        bytes: Option<&[u8]>,
        big_clr: bool,
    ) -> Result<()> {
        ...
        match bytes {
            ...
            Some(data) => {
                ...
                if len == 0 {
                    ...
                    let max_chunk = if big_clr {
                        CHUNK_SIZE
                    } else {
                        length::MAX_SHORT as usize
                    };
                    let mut offset = 0;
                    while offset < len {
