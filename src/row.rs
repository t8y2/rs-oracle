...
use crate::types::{
    ...
    decode_oracle_timestamp, decode_rowid, IntervalDayToSecond, IntervalYearToMonth, LobValue,
    OracleDate, OracleNumber, OracleTimestamp, OracleVector, RefCursor, RowId,
};
...
pub enum Value {
    ...
    IntervalYm(IntervalYearToMonth),
    /// INTERVAL DAY TO SECOND value
    IntervalDs(IntervalDayToSecond),
}
...
impl Value {
    ...
    /// Try to get as an INTERVAL YEAR TO MONTH
    pub fn as_interval_ym(&self) -> Option<&IntervalYearToMonth> {
        match self {
            Value::IntervalYm(v) => Some(v),
            _ => None,
        }
    }

    /// Try to get as an INTERVAL DAY TO SECOND
    pub fn as_interval_ds(&self) -> Option<&IntervalDayToSecond> {
        match self {
            Value::IntervalDs(v) => Some(v),
            _ => None,
        }
    }
}
...
impl std::fmt::Display for Value {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Value::IntervalYm(iv) => write!(f, "+{}-{:02}", iv.years, iv.months),
            Value::IntervalDs(iv) => write!(
                f,
                "+{} {:02}:{:02}:{:02}.{:09}",
                iv.days, iv.hours, iv.minutes, iv.seconds, iv.nanoseconds
            ),
        }
    }
}
...
impl<'a> RowDataDecoder<'a> {
    ...
    pub fn decode_row(&mut self, buf: &mut ReadBuffer, previous_row: Option<&Row>) -> Result<Row> {
        ...
        let names = self
            .column_names
            .get_or_insert_with(|| Arc::new(self.columns.iter().map(|c| c.name.clone()).collect()));
        Ok(Row {
            values,
        ...
        if column.buffer_size == 0 {
            match column.oracle_type {
                OracleType::Long | OracleType::LongRaw | OracleType::Urowid => {
                    // LONG/LONG RAW rows carry their own length prefix in the wire data.
                    // The column's max_size tells us the maximum possible allocation.
                }
