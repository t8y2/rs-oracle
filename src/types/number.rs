...
/// Fast-path: try to decode an Oracle NUMBER directly to i64 or f64,
/// avoiding the String intermediate allocation. Returns None if the
/// value requires full string precision (e.g., very large integers or
/// exact decimals).
///
/// This is the hot-path optimization: most NUMBER columns in real
/// schemas (IDs, counts, amounts) fit in i64 or f64 and don't need
/// arbitrary-precision string representation.
pub fn try_decode_number_fast(data: &[u8]) -> Option<(f64, bool)> {
    if data.is_empty() {
        return None;
    }
    ...
    if data.len() == 1 {
        return if is_positive { Some((0.0, true)) } else { None };
    }
    ...
    // Accumulate mantissa digits and track decimal point
    ...
    for i in 0..mantissa_len {
        ...
        if d1 == 0 && digit_count == 0 {
            decimal_point_index -= 1;
        } else if d1 == 10 {
            if digit_count < 40 { digits_buf[digit_count] = 1; digit_count += 1; }
            if digit_count < 40 { digits_buf[digit_count] = 0; digit_count += 1; }
            decimal_point_index += 1;
        } else if d1 != 0 || digit_count != 0 {
            if digit_count < 40 { digits_buf[digit_count] = d1; digit_count += 1; }
        }

        if d2 != 0 || i < mantissa_len - 1 {
            if digit_count < 40 { digits_buf[digit_count] = d2; digit_count += 1; }
        }
    }

    // Trim trailing zeros
    while digit_count > 0 && digits_buf[digit_count - 1] == 0 {
        if (digit_count as i32) <= decimal_point_index { break; }
        digit_count -= 1;
    }

    let is_integer = decimal_point_index as usize >= digit_count;

    if is_integer && digit_count <= 19 {
        // Fits in i64: compute with overflow checks
        let mut val: i64 = 0;
        for i in 0..digit_count {
            val = val.checked_mul(10)?.checked_add(digits_buf[i] as i64)?;
        }
        let trailing_zeros = decimal_point_index as usize - digit_count;
        for _ in 0..trailing_zeros {
            val = val.checked_mul(10)?;
        }
        if !is_positive { val = val.checked_neg()?; }
        return Some((val as f64, true));
    }

    // Fast f64 path: compute with powers of 10
    if digit_count <= 17 {
        let mut val: f64 = 0.0;
        for i in 0..digit_count {
            val = val * 10.0 + digits_buf[i] as f64;
        }
        let scale = decimal_point_index as i32 - digit_count as i32;
        val *= 10_f64.powi(scale);
        if !is_positive { val = -val; }
        return Some((val, is_integer));
    }
